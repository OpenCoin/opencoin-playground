...
```{note}
The examples are produced by the [run_protocol.py](../run_protocol.py) script.
```
...
- **[additional_info](fields.md#additional_info)**: A field where the issuer can store additional information about the currency. Free text for humans.  *(String)*
- **[cdd_expiry_date](fields.md#cdd_expiry_date)**: The date the CDD expires.  *(String)*
- **[cdd_location](fields.md#cdd_location)**: Location to download the CDD from.  *(URL)*
...
These are the "innards" of [Coin](#coin). The mint key id is only a helper so that the corresponding key for the signature of the coin can be found faster. The same is true for the denomination - an implementation should take the coins value from the [MintKey](#mintkey), and not from the contents of the Payload, because the values in the Payload are user defined.
...
Request blinds to be signed. The [Blinds](#blind) hold the information which mint keys are to be used for minting.
...
This delivers the [BlindSignatures](#blindsignature). The client will derive the signature for the [Coins](#coin) by unblinding the BlindSignatures.
...
One solution would be to require the recipient to give change back to the client. This would complicate the protocol, and would just shift the problem to the recipient. Another approach is to allow partial spending coins, but this again makes the protocol more complicated.[^partial]
...
Also see [](operations.md#tokenizing) for a sample implementation of a solution.
